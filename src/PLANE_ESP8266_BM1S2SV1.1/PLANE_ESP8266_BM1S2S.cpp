...
void loop()
{
  ...
  if (millis() - h_t > 3000)
  {
    ...
    for (int i = 0; i < 6; i++)
    {
      rc_address[i] = broadcast_address[i];
    }
    analogWrite(RIGHT_MOTOR_PIN, 0);
    analogWrite(LEFT_MOTOR_PIN, 0);
    servo_yaw.write(90 + yaw_correct);
    servo_pitch.write(90 - pitch_correct);
    servo_roll.write(90 + roll_correct);
  }
  if (dmpReady)
  {
